...
class MessageBody(BaseModel):
    ...
    attachments: Optional[List[Union[AttachmentButton, Audio, Video, File, Image, Sticker, Share, Location]]] = Field(
        default_factory=list
    )  # type: ignore

    markup: Optional[List[Union[MarkupLink, MarkupElement]]] = Field(default_factory=list)  # type: ignore
...
class Message(BaseModel):
    ...
    async def forward(
        ...
        return await self.bot.send_message(
            ...
            user_id=user_id,
            attachments=attachments,
            link=NewMessageLink(type=MessageLinkType.FORWARD, mid=self.body.mid),
            notify=notify,
            parse_mode=parse_mode,
        )
    ...
    async def pin(self, notify: bool = True):
        ...
        return await self.bot.pin_message(chat_id=self.recipient.chat_id, message_id=self.body.mid, notify=notify)
