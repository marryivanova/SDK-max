...
class Handler:
    ...
    def __init__(self, *args, func_event: Callable, update_type: UpdateType, **kwargs):
        """
        ...
        for arg in args:
            if isinstance(arg, MagicFilter):
                ...
                logger.info(f"Обнаружен неизвестный фильтр `{arg}` при " f"регистрации функции `{func_event.__name__}`")
